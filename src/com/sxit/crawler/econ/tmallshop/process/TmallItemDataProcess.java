 ...
 */
public class TmallItemDataProcess extends CrawlProcess{

	private static Logger log = LoggerFactory.getLogger(TmallItemDataProcess.class);
	...
	public TmallItemDataProcess(UserAgentProvider userAgentProvider,
	...
	public void process(Map<String, Object> param) {
		...
		String sql = "SELECT * FROM ( SELECT A.*, ROWNUM RN FROM (SELECT * FROM TBAS_SHOP_TMALL_ITEMIDS where PRC_FLAG = 0 order by crt_date) A WHERE ROWNUM <= ?) WHERE RN >= ?";
		int pageSize = 20;
	...
	private void crawlItemDataByIds(LinkedHashMap<String, String> idMaps) throws Exception{
		...
		if (itemsListGetResponse.isSuccess()) {
			...
			if (!CollectionUtils.isEmpty(rows)) {
				JdbcTemplate jdbcTemplate = (JdbcTemplate)BeanHelper.getBean("jdbcTemplate");
				DatatableOperator datatableOperator = new DatatableOperator(((TmallShopItemCrawlerModule)crawlModule).tmallItemDatatableConfig, jdbcTemplate);
