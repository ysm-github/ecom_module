...
public class CrawlTmallShop2 {
	...
	private static Logger log = LoggerFactory.getLogger(CrawlTmallShop2.class);
	public static void main(String[] args) {
		...
		for (int i = 0; i < pageSize; i++) {
			try {
				...
				log.info("pagesize----->"+(i+1));
			} catch (Exception e) {
				log.error(e.getMessage());
			}
			
		}
		log.info("****************************Tmall shop over******************************");
	}
	...
	private static void processPage(FetchEntry fetchEntry) {
		...
		if (!CollectionUtils.isEmpty(shopElts)) {
			log.info("SHOP SIZE:"+shopElts.size());
			for (Element shopElt : shopElts) {
				try {
					...
					if (StringUtils.isNotBlank(shopId)) {
						...
						log.info(shopElt.html());
						log.info(shopElt.tagName());
//						log.info((null != shopElt.select("textarea.ks-datalazyload")));
						log.info("------------------------------------------------");
					}
				} catch (Exception e) {
					log.error(e.getMessage());
