...
public class DataTransTaobao {
	...
	public static void main(String[] args) {
		...
		DatatableConfig datatableConfig = initDatatableConfig("datatrans_taobao_datatable_config.xml");
		JdbcTemplate jdbcTemplate = (JdbcTemplate)BeanHelper.getBean("jdbcTemplate");
		String sql = "SELECT * FROM ( SELECT A.*, ROWNUM RN FROM (select * from TBAS_SHOP_TAOBAL_ITEM where cid = '1512' order by id) A WHERE ROWNUM <= ?) WHERE RN >= ?";
		String countSql = "select count(*) from TBAS_SHOP_TAOBAL_ITEM where cid = '1512'";
